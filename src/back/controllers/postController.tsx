...
import { Post, RequestWithFile, ErrorResponse, SuccessResponse } from '../types.js';

// Получение всех постов пользователя
export const getUserPosts = async (req: Request, res: Response<SuccessResponse | ErrorResponse>) => {
  try {
    ...
    const postList: Post[] = posts.map(post => post.toObject() as Post);
...
export const getAllPosts = async (req: Request, res: Response<SuccessResponse | ErrorResponse>) => {
  try {
...
export const createPost = async (req: RequestWithFile, res: Response<SuccessResponse | ErrorResponse>) => {
  ...
  try {
    ...
    return res.status(201).json({ success: true, post: post.toObject() as Post });
  } catch (error: unknown) {
...
// Удаление поста
export const deletePost = async (req: Request, res: Response<ErrorResponse | SuccessResponse>) => {
...
export const getPostById = async (req: Request, res: Response<SuccessResponse | ErrorResponse>) => {
  const { postId } = req.params;
  ...
  try {
    ...
    return res.status(200).json({ success: true, post: post.toObject() as Post });
  } catch (error: unknown) {
...
// Обновление поста
export const updatePost = async (req: Request, res: Response<SuccessResponse | ErrorResponse>) => {
  const { postId } = req.params;
